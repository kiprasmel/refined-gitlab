...
async function gitlabSessionCookieSync(): Promise<void> {
	try {
		...
		// if (!isSignedInCookie) {
		// 	const msg = "Cannot update cookies (`known_sign_in` cookie was falsy)";
		// 	console.error(msg);

		// 	await sendMessage(tab.id, {
		// 		gitlabSessionToken: undefined,
		// 		isSignedInCookie: undefined,
		// 		success: false,
		// 		reason: msg,
		// 	});
	...
	browser.runtime.onMessage.addListener(
		async (
			...
			try {
				...
				/**
				 * TODO: This ain't good, we prolly shouldn't override the CSRF stuff
				 */
				await browser.cookies.set({
					url: tab.url,
